...
describe.skipIf(skipReason !== null)("process_transaction RPC", () => {
  ...
  afterAll(async () => {
    ...
    // Clean up activity_logs generated by the triggers during test
    await supabase
      .from("activity_logs")
      .delete()
      .like("details->>product_id", `%`)
      .or(createdProductIds.map((id) => `record_id.eq.${id}`).join(","));
  });

  async function createTestSite() {
    const name = `${TEST_PREFIX}site_${crypto.randomUUID()}`;
  ...
  it("rejects out transaction without a site (SITE_REQUIRED)", async () => {
    const product = await createTestProduct(50);
    ...
    expect(data).toBeNull();
    expect(error?.message).toContain("SITE_REQUIRED");
  });
