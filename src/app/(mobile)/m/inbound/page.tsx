...
export default async function MobileInboundPage() {
  ...
  const { data, count } = await supabase
    .from("transactions")
    .select("id, type, quantity, created_at, created_by, products!inner(name, category, unit)", { count: "exact" })
    .eq("type", "in")
  ...
  return (
    <div className="space-y-4">
      {/* KPI header */}
      <div className="rounded bg-primary text-primary-foreground p-4">
        ...
        <p className="text-xs text-primary-foreground/60 mt-0.5">전체 입고 건수</p>
      </div>
              ...
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    {tx.products?.category ?? ""} · {tx.created_by ? profileMap.get(tx.created_by) ?? "-" : "시스템"}
                  </p>
                </div>
